...
You only need a BING API key, if you intend to run the registry_creator.py script which is the last pipeline step and creates the address-level PV registry. In case you are only interested in finding the PV locations and the PV polygons, you do neither need to run registry_creator.py nor obtain a Bing API key.
---
